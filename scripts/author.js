...
downloader.addEventListener('click', function(){
    var authService2 = firebase.auth().currentUser;
    if(authService2 != null){
        ...
        descarga.getDownloadURL().then(function(url){
            ...
            console.log(url);
            document.getElementById('prueba').innerHTML = '<a href="' + url + '"> Link de descarga </a> <style>  prueba { color: #FF0000; } </style>'
        ...
        $('#incorrecto').text("ERROR. Debe loguearse para poder obtener el enlace.");
...
function subidaArchivo() {
    ...
    if (authService != null) {
        ...
        console.log("Tienes que iniciar sesión.");
        document.getElementById('necesita_loguearse').style.display="block";
        $('#necesita_loguearse').addClass('invalid');
        $('#necesita_loguearse').text("ERROR. Debe loguearse para poder subir un archivo.");
        $('#campoarchivo').value = ' ';
    }
