...
function registerMail() {
    if (validar_email() === false) {
        ...
        $('#register-mail').text("Invalid mail error. Check the data. Example: mail@mail.com");
    } else {
...
function loginReady() {
    ...
    if (userLogin == null) {
        ...
        if (regex.test($('#login-email').val())) {    //compruebas el correo
            ...
            $('#loginMail').text("Enter your email in the previous field.");
        } else {
            console.log("Incorrecto");
            $('#loginMail').addClass('invalid');
            $('#loginMail').text("Invalid mail error. Check the data. Example: mail@mail.com");
        }
    }
}
